...
function TableDemo(props: any) {
  ...
  const handleSortColumn = (
    sortColumn: SetStateAction<string | undefined>,
    sortType: SetStateAction<"asc" | "desc" | undefined>
  ) => {
  ...
  const userDetails = async (org: string) => {
    const result = await apiFetch(`/users/users/${org}`, {
      method: "GET",
      headers: {
    ...
    <>
      ...
      >
        ...
        <Column flexGrow={1} align="center">
          ...
          <Cell dataKey="user_list.length">
            {(rowData) => rowData.user_list.length.toString()}
          </Cell>
        </Column>
      ...
      <Modal open={open2} onClose={handleClose2}>
        <Modal.Header>
          <Modal.Title>
            <strong style={{ color: "black" }}>Users in {orgName}</strong>{" "}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
